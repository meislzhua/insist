...
export default class GoalPage extends React.Component {
      ...
      switch (goal.repetition) {
        ...
        case 'month':
          ...
          while (startTime.isSameOrBefore(endTime, 'day')) {
            count += (this.state.historyIndex[this.getDateKey({ date: startTime })] || []).filter(
        ...
        case 'appoint_month':
          if (
            (goal.repetition === 'appoint_month' ? date.date() : date.isoWeekday()) === goal.appoint
          ) {
            count = (this.state.historyIndex[this.getDateKey({ date })] || []).filter(
              (history: GoalHistory) => history.goal.objectId === goal.objectId,
            ).length;
            isVisible = !count;
          }
  ...
  async refreshGoalHistory() {
    ...
    const historyIndex = (await Dao.goal.listGoalsHistory({ startTime, endTime })).reduce(
      (p: any, v) => {
        // 按照日期建立数组
        ...
        <div className={styles.container}>
          ...
          <div className={styles.addItemBox}>
            <Button
              ...
              onClick={() =>
                this.goalAddBox?.toggleAddGoalShow({ day: this.goalCalendar?.nowDate() })
              }
            >
              <PlusOutlined />
            </Button>

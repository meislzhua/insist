...
export default class GoalAddBox extends React.Component<GoalAddBoxProps> {
  ...
  event_finishGoal({isSuccess, goal, content}: { isSuccess: boolean, goal: Goal, content?: string }) {
    return Modal.confirm({
      ...
      onOk: async () => {
        ...
        await Dao.goal
          .finishGoal({goal, isSuccess, content, appointDate: appointDate || undefined})
  ...
  render() {
    ...
    } else if (goal.repetition === "day") infos.push(<Tag color="#d48806">每日{goal.repetitionCount}次</Tag>)
    else if (goal.repetition === "week") infos.push(<Tag color="#08979c">每周{goal.repetitionCount}次</Tag>)

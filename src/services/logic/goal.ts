...
import type { GoalHistory } from '@/services/Dao/struct/goal/GoalHistory';
...
export class GoalLogic {
  static getShowGoalInDay({
    ...
    if (today.isAfter(date, 'day')) return [];
    const targetGoals = goals.reduce((p: Goal[], goal: Goal) => {
      let isVisible = false;
      let count;
      // day,week,month 计算所需变量
      let startTime = today;
      let endTime = today;
      let distanceCalculateDate;
      switch (goal.repetition) {
        case 'once':
          isVisible = date.isSame(today, 'day');
        ...
        case 'month':
          ...
          isVisible = !!(
            goal.repetitionCount &&
            goal.repetitionCount - count > date.diff(distanceCalculateDate, 'day')
          );
          break;
        case 'appoint_week':
    ...
    return GoalLogic.goalOrder({ goals: targetGoals });
  }
